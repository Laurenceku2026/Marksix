...
with st.sidebar:
    ...
    with st.expander("中央趋向定理", expanded=False):
        ...
        st.markdown("- 标准差: 约35")
        st.markdown("- 约68%的组合和值在140-210之间")
    with st.expander("冷热码计算公式", expanded=False):
        st.markdown("Score = 0.3*z_freq + 0.3*z_absence + 0.2*z_short + 0.2*recent")
    with st.expander("连号/跳号概率", expanded=False):
        st.markdown("- 至少一对连号: 55.6%")
        st.markdown("- 至少一对跳号: 65%")
    with st.expander("奖金结构", expanded=False):
        st.markdown("| 等级 | 匹配 | 奖金 |")
        st.markdown("|------|------|------|")
        st.markdown("| 第1组 | 6 | 45%基金 |")
        st.markdown("| 第2组 | 5+特 | 15%基金 |")
        st.markdown("| 第3组 | 5 | 40%基金 |")
        st.markdown("| 第4组 | 4+特 | $9,600 |")
        st.markdown("| 第5组 | 4 | $640 |")
        st.markdown("| 第6组 | 3+特 | $320 |")
        st.markdown("| 第7组 | 3 | $40 |")
...
fig = px.line(sum_df, x="期次", y="和值(7码)", title=f"最近{show_periods}期和值走势 (7个号码)")
...
with col2:
    selected_date = st.date_input("开奖日期（用于Random Seed）", value=datetime.now())
...
if st.button("生成智能投注", type="primary"):
    ...
    bets = generate_bets_by_strategy(
        draws, scores, hot_numbers, cold_numbers, num_bets, strategy,
    ...
    **预测信息**
    ...
    - 共生成 {num_bets} 注推荐号码
    - 每注 {num_count} 个号码
    - 理论期望和值: {get_target_sum_by_numbers_count(num_count)}
    """)

    if num_count == 7:
        st.caption("提示: 7个号码包含6个正码和1个特码")
...
if st.session_state.get("generated_bets") is not None:
    ...
    with st.expander("📋 点击查看数据格式说明", expanded=False):
        st.markdown("""
        格式：期次 日期 B1 B2 B3 B4 B5 B6 B7
        示例：
